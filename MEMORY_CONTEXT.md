...
- ⏳ Phase 4 (pagination), LINE OTP, top-up ledger UI, **Phase 6 analytics** are on `develop` but **not yet released** — CHANGELOG `[1.1.0]`=Phase 3, `[Unreleased]`=the rest. Next release = v1.2.0 (`npm run release:minor`).
- ⏳ LINE/email **payment reminders** (deferred "Phase 5/B") and **referral** program (deferred "Phase 7/C")
- ⏳ **Platform-admin analytics** (cross-shop) — only per-shop owner analytics shipped so far
...
### ✅ Phase 6 — Shop analytics dashboard (DONE, per-shop owner)
- **Goal:** give owners insight beyond raw counts.
- **Shipped:** `/shop/analytics` — stat tiles (stamps/redemptions/active customers/redemption rate/new customers), daily trend chart (**recharts**), per-stamp-type & per-branch breakdown, top customers, 7/30/90-day range. Read-model `IAnalyticsRepository` (group-by Bangkok day via `strftime(...,'+7 hours')`), pure `analytics.ts` service, `GetShopAnalyticsUseCase`. Top-up ledger UI also shipped (Phase 4 batch).
- **Not in phase:** cross-shop **platform-admin** analytics, CSV/PDF export, period-over-period.
